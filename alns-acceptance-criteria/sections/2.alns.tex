...
ALNS was introduced by \citet{ropke2006adaptive} and extends the LNS metaheuristic first proposed by \citet{Shaw98}.
...
\secrev{
The LNS and ALNS has similarities to the method known as iterated greedy, first proposed by \citet{RuSt07}.
...
\citet{Gl00} points out that alternating constructive and destructive phases corresponds to a one-sided strategic oscillation.
Strategic oscillation refers to a process where a search is strategically driven to go back and forth between the set of feasible solutions and a set of infeasible solutions \citep{GlLa97}.
...
Finally, the ruin and recreate heuristic, proposed in \citep{Schrimpf00} is also very similar to LNS and ALNS in the sense that it improves an initial solution by repeatedly destroying (ruining) and repairing (recreating) the current solution.
}