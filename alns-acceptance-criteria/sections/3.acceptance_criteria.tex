...
The criterion was first introduced by \autocite{burke2008late,burke2012late}.
...
As proposed by \autocite{burke2012late}, the criterion can be emended to accept $x'$ if \emph{either} it is better than the current solution $L$ iterations ago, \emph{or} it is better than the current solution at the present iteration.
...
With the Threshold Acceptance (TA) criterion introduced by \autocite{dueck1990threshold} and presented in \Cref{alg:ta}, a \rev{new} solution is accepted if the gap between the \rev{new} and the current solution is smaller than a threshold $T$.
...
It was originally introduced by \autocite{kirkpatrick1983optimization} and it was used with the ALNS since its debut by \autocite{ropke2006adaptive}.
...
We refer to this method, introduced as the default acceptance criterion for ALNS by \autocite{ropke2006adaptive}, simply as \textbf{Exponential Simulated Annealing}.
A variant of this method has been proposed by \autocite{pisinger2007general}, where the authors noticed that the start and end temperature values can be sensitive to the size of the instance.
...
Since \autocite{pisinger2007general} only considered the case where $M = 1$, we take this as the base case upon which we build the following additional variations.
...
Another common variant is SA with reheating, discussed by \autocite{connolly1992general}.
...
With the Great Deluge (GD) criterion, introduced by \autocite{dueck1993new} and presented in \Cref{alg:gd}, a \rev{new} solution is accepted only if its cost is smaller than a threshold, called the \emph{water level}.
...
The NLGD was introduced by \autocite{landa2008great} for a course timetabling problem;
...
WA was employed as the acceptance criterion in an ALNS-based metaheuristic for the Partition Colouring Problem (a generalisation of the VCP) by \autocite{furini2016branch}.
